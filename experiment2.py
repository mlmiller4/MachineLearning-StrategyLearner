...
if __name__ == "__main__":
        ...
        s3 = StrategyLearner(impact=0.10)

        sv = 100000
        insample_sd = dt.datetime(2008, 1, 1)
        insample_ed = dt.datetime(2009, 12, 31)

        sl.addEvidence(symbol='JPM', sd=insample_sd, ed=insample_ed, sv=sv)
        s2.addEvidence(symbol='JPM', sd=insample_sd, ed=insample_ed, sv=sv)
        s3.addEvidence(symbol='JPM', sd=insample_sd, ed=insample_ed, sv=sv)

        results_s1 = sl.testPolicy(symbol='JPM', sd=insample_sd, ed=insample_ed, sv=sv)
        results_s2 = s2.testPolicy(symbol='JPM', sd=insample_sd, ed=insample_ed, sv=sv)
        results_s3 = s3.testPolicy(symbol='JPM', sd=insample_sd, ed=insample_ed, sv=sv)


        # Get portfolio values using compute_portvals
        # strategy_portvals = compute_portvals(strategy_results, sv, 0, 0)
        # strategy_portvals = strategy_portvals / strategy_portvals[0]

        portvals05 = compute_portvals(results_s1, sv, 0, 0)
        portvals05 = portvals05 / portvals05[0]
        portvals10 = compute_portvals(results_s2, sv, 0, 0)
        portvals10 = portvals10 / portvals10[0]
        portvals15 = compute_portvals(results_s3, sv, 0, 0)
        portvals15 = portvals15 / portvals15[0]

        fig, ax = plt.subplots()

        ax.plot(portvals05, 'k', label="Impact = 0.01")
        ax.plot(portvals10, 'b', label="Impact = 0.05")
        ax.plot(portvals15, 'g', label="Impact = 0.10")

        ax.set(xlabel="Jan. 1, 2008 - Dec. 31, 2009", ylabel="Value (Normalized)",
               title="Comparison of Portfolio Performance During In-Sample Period \n between Impact Values of 0.01, 0.05, & 0.10")

        ax.set_xlim(insample_sd, insample_ed)
        ...
        plt.grid()
        #plt.show()
        fig.savefig('experiment2_insample.png')
        plt.close(fig)

        """ Repeat with out-of-sample time period
        ------------------------------------------------------------------------------------------------"""
        outofsample_sd = dt.datetime(2010, 1, 1)
        outofsample_ed = dt.datetime(2011, 12, 31)

        sl = StrategyLearner(impact=0.01)
        s2 = StrategyLearner(impact=0.05)
        s3 = StrategyLearner(impact=0.10)

        sl.addEvidence(symbol='JPM', sd=outofsample_sd, ed=outofsample_ed, sv=sv)
        s2.addEvidence(symbol='JPM', sd=outofsample_sd, ed=outofsample_ed, sv=sv)
        s3.addEvidence(symbol='JPM', sd=outofsample_sd, ed=outofsample_ed, sv=sv)

        results_s1 = sl.testPolicy(symbol='JPM', sd=outofsample_sd, ed=outofsample_ed, sv=sv)
        results_s2 = s2.testPolicy(symbol='JPM', sd=outofsample_sd, ed=outofsample_ed, sv=sv)
        results_s3 = s3.testPolicy(symbol='JPM', sd=outofsample_sd, ed=outofsample_ed, sv=sv)
        ...
        ax.set_xlim(outofsample_sd, outofsample_ed)
        ax.title.set_fontsize(14)
        ax.xaxis.label.set_fontsize(14)
        ax.yaxis.label.set_fontsize(14)
        ...
        plt.grid()
        #plt.show()
        fig.savefig('experiment2_outofsample.png')
        plt.close(fig)
